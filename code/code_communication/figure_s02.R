## ---------------------------
##
## Script name: figure_s02
##
## Purpose of script:
##    Create figure of distributions of population density, poverty, and 
...
dac_data$dac <- factor(
  dac_data$dac,
  levels = c("cejst", "ces", "ces_adj", "eji", "univariate")
)
labels = c("CEJST", "CES", "CES+", "EJI", "Trivariate")

# Create legend
legend <- get_legend (ggplot(dac_data  %>% filter(dac!= "total"), 
                             aes(x = black_prop, y = hispanic_prop, group = dac, color = dac)) +
  geom_line()+
  theme_classic() +
...
figure_s2_plots <- cowplot::plot_grid(part_a + theme(legend.position="none"), 
...
# add common legend
figure_s2 <- cowplot::plot_grid(figure_s2_plots, legend, nrow = 2, rel_heights = c(1, .1))
print(figure_s2)
cowplot::ggsave2("outputs/figures/fig_s02.png", figure_s2,
                 width = 7,
                 height = 6,
                 units = c("in"))
