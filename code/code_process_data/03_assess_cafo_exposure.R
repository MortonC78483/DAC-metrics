...
# cafos <- read_sf("data/CAFO_data/Regualted_Facility_Report_Detail.csv")
# tract <- tracts(
#   state = "CA",
#   year = 2019
# ) %>%
...
#   mutate(Weight = ((Population-min(Population))/(max(Population)-min(Population)))*(2-.1)+.1) # rescale to be from .1 to 2
...
#     weights <- cafos_in_tract_filtered$Weight
