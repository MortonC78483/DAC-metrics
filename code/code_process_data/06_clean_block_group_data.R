## ---------------------------
##
## Script name: 06_clean_block_group_data
##
## Purpose of script:
##    This script creates a block-group level dataset.
##
...
# Load data ---------------------------
block_group <- read_csv("data/data_interim/CA_census_block_group.csv")
...
merged <- merge(block_group, ces_scores, by.x = "tract", by.y = "GEOID") %>%
  merge(cejst_scores, by.x = "tract", by.y = "GEOID", all = T) %>%
  merge(eji_scores, by.x = "tract", by.y = "GEOID", all = T) %>%
...
# Export dataset ---------------------------
write_csv(merged, "data/data_processed/metrics_block_group.csv")
