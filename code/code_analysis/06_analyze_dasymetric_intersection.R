...
# Load data ---------------------------
buffer <- read.csv("data/data_interim/dasymetric_intersection_tract_1km.csv") %>%
...
intersect_ces = sum(merged[merged$ces_dac == 1,]$area_intersect, na.rm = T)/sum(merged[merged$ces_dac == 1,]$area_tract, na.rm = T)
intersect_ces_adj = sum(merged[merged$ces_dac_adj == 1,]$area_intersect, na.rm = T)/sum(merged[merged$ces_dac_adj == 1,]$area_tract, na.rm = T)
intersect_univariate = sum(merged[merged$univariate_dac == 1,]$area_intersect, na.rm = T)/sum(merged[merged$univariate_dac == 1,]$area_tract, na.rm = T)
intersect_eji = sum(merged[merged$eji_dac == 1,]$area_intersect, na.rm = T)/sum(merged[merged$eji_dac == 1,]$area_tract, na.rm = T)
intersect_cejst = sum(merged[merged$cejst_dac == 1,]$area_intersect, na.rm = T)/sum(merged[merged$cejst_dac == 1,]$area_tract, na.rm = T)
...
no_pop <- read_csv("data/data_processed/metrics_tract.csv") %>%
